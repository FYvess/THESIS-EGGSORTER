...
l_gpio.setup(18, l_gpio.OUT)
l_gpio.output(18, l_gpio.HIGH)
l_gpio.setup(17, l_gpio.OUT)
l_gpio.output(17, l_gpio.LOW)
...
class c_main(c_databank):
    def __init__(self,master):
        ...
        self.button4=tkinter.Button(self.frame2,text="Auto Mode",width=15,height=5,bg='green',command=lambda:self.f_setauto())
    ...
    def f_ledindic(self):
        while c_databank.g_auto==True:
            time.sleep(2.5)
            l_gpio.output(18, l_gpio.HIGH)
            l_gpio.output(17, l_gpio.LOW)
            time.sleep(2.5)
            l_gpio.output(18, l_gpio.LOW)
            l_gpio.output(17, l_gpio.HIGH)
            time.sleep(6.8)
            l_gpio.output(18, l_gpio.HIGH)
            l_gpio.output(17, l_gpio.LOW)
            time.sleep(0.2)
    ...
    def f_setauto(self):
        ...
        if(lv_autostatus==0):
            ...
            if hasattr(self, 'g_camera_thread') and self.g_camera_thread is not None and self.g_camera_thread.is_alive():
                ...
                return
            self.g_camera_thread = threading.Thread(target=self.f_ledindic)
    ...
    def f_processimage(self):
        ...
        if(self.lv_error==1):
            ...
            self.text1.insert(self.text1.index('end'),"system -> egg size ["+self.lv_eggsize+"]\n")
            self.f_resetserial()
            ...
            if(c_databank.g_serial.in_waiting>0):
                ...
                if(self.lv_receiveddata==b'\n'):
                    ...
                    if(c_databank.g_data[0]=="2005"):
                        # self.text1.insert(self.tefxt1.index('end'), 'system ->' + c_databank.g_data[6]+'\n')
                        
                        self.text1.insert(self.text1.index('end'),'system -> \t-------------------------------------------\n')
                        self.text1.insert(self.text1.index('end'),'system -> \t\t\tSTATISTICAL REPORT\n')
                        self.text1.insert(self.text1.index('end'),"system -> \t\tSmall Qty :\t\t\t"+c_databank.g_data[1]+'\n')
                        self.text1.insert(self.text1.index('end'),"system -> \t\tMedium Qty:\t\t\t"+c_databank.g_data[2]+'\n')
                        self.text1.insert(self.text1.index('end'),"system -> \t\tLarge Qty:\t\t\t"+c_databank.g_data[3]+'\n')
                        self.text1.insert(self.text1.index('end'),"system -> \t\tExtra Large Qty :\t\t\t"+c_databank.g_data[4]+'\n')


                        self.text1.insert(self.text1.index('end'),"system -> \t\tJumbo Qty :\t\t\t"+c_databank.g_data[5]+'\n')
                        self.text1.see(tkinter.END)
